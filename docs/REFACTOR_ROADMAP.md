...
- `tracker_runtime_receive.record_tracker_receive_unlocked(...)` now expires pending deliveries in a `finally` block so receive-path expiration is guaranteed even when injected packet-record callbacks raise.
