...
_STANDALONE_ADVENTURE_LOCAL_NODE_ID = "!7a000002"
_STANDALONE_ADVENTURE_SESSION_PEER_HEX_CHARS = 16
_DEFAULT_MAX_STANDALONE_ADVENTURE_SESSIONS = 256
...
def _session_peer_id(session_id: str) -> str:
    ...
    return f"!{digest[:_STANDALONE_ADVENTURE_SESSION_PEER_HEX_CHARS]}"


class StandaloneAdventureService:
    def __init__(
        self,
        *,
        now_unix_fn=time.time,
        max_sessions: int = _DEFAULT_MAX_STANDALONE_ADVENTURE_SESSIONS,
    ) -> None:
        self._game = AdventureGame()
    ...
    def play(self, *, text: object, session_id: object = None) -> dict[str, object]:
        ...
        with self._lock:
            ...
            is_start = clean_text.lower() in {"adventure", "adv", "restart"}
            if not active_before and is_start and self._game.active_session_count() >= self._max_sessions:
                return {
                    "ok": False,
                    "error": "Standalone Adventure is at capacity. Try again shortly.",
                    "session_id": clean_session_id,
                    "active_session": False,
                }
            if not active_before and not is_start:
                return {
                    "ok": False,
                    "error": "No active standalone Adventure session. Send 'adventure' to start.",
                    "session_id": clean_session_id,
                    "active_session": False,
                }
            result = self._game.try_handle_message(
                text=clean_text,
                from_id=peer_id,
                to_id=_STANDALONE_ADVENTURE_LOCAL_NODE_ID,
                local_node_id=_STANDALONE_ADVENTURE_LOCAL_NODE_ID,
                now_unix=now_unix,
                enabled=True,
            )
...
def build_standalone_adventure_service() -> StandaloneAdventureService:
    return StandaloneAdventureService()


__all__ = [
    "StandaloneAdventureService",
    "build_standalone_adventure_service",
]
