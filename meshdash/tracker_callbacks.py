    ...
    *,
    ...
    def _set_delivery_state(
        ...
        *,
        ...
        if changed and callable(on_change_fn):
            on_change_fn()
        return changed
    ...
    def _expire_pending_deliveries() -> None:
        ...
        def _persist_expired_delivery(entry: dict[str, object]) -> None:
            nonlocal changed
            changed = True
            _persist_delivery_update(entry)
        ...
        _expire_tracker_pending_deliveries_helper(
            ...
            utc_now_fn=utc_now_fn,
            now_unix_fn=now_unix_fn,
            on_expire_fn=_persist_expired_delivery,
        )
        if changed and callable(on_change_fn):
            on_change_fn()
