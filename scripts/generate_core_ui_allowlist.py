#!/usr/bin/env python3
"""Generate the public core-ui release allowlist.

This script traces local Python imports from runtime entrypoints and adds the
frontend templates shipped by the public branch so the allowlist can stay
explicit without shipping the entire repository.
"""
...
PUBLIC_PROFILE = "core-ui"
DEFAULT_OUTPUT = REPO_ROOT / ".public-release" / "allowlists" / f"{PUBLIC_PROFILE}.allowlist"
...
def _collect_profile_assets() -> set[Path]:
    ...
    assets = {
        ...
        for template_name in _template_parts_for_profile()
    }
    assets.update(
        (REPO_ROOT / "meshdash" / "assets" / template_name)
...
def _build_allowlist_lines() -> list[str]:
    ...
    header = [
        f"# Auto-generated core runtime allowlist for profile: {PUBLIC_PROFILE}",
...
def _validate_minimal_tree(allowlist_lines: list[str]) -> None:
    entries = [
        line.strip()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate the public core-ui allowlist.")
    parser.add_argument(
        "--output",
    ...
    new_lines = _build_allowlist_lines()
    new_text = "\n".join(new_lines)
    ...
    if args.validate:
        _validate_minimal_tree(new_lines)
